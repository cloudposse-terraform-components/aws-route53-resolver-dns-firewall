...
# Component: `aws-route53-resolver-dns-firewall`
...
- **Domain Filtering**: Creates domain lists and firewall rules to allow or block DNS queries to specific domains
- **VPC Association**: Associates firewall rule groups with VPCs for DNS-level network protection
- **Query Logging**: Optionally logs all DNS queries to an S3 bucket for auditing and analysis
- **Fail-Open/Fail-Closed**: Controls whether DNS resolution continues or stops if the firewall fails
...
| <a name="input_vpc_component_name"></a> [vpc\_component\_name](#input\_vpc\_component\_name) | The name of a VPC component where the DNS Firewall is provisioned | `string` | n/a | yes |
